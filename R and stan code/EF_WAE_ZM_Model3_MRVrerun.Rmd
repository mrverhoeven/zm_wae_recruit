...
Load in Data
```{r, message=FALSE}
EF_WAE_ZM_data <- read_csv("R and stan code/Data/EF_DATA_MRVrerun.csv")
...
```{r}
write.csv(final.dat3, "R and stan code/Data/NN_EF_WAE_ZM_Model_MRVrerun.csv")
```
...
# Effort offset term (maybe multiply eff_seconds by the number_of_netters?) OCt Diagnostics, removed N netters
dat <- dat %>% 
...
out <- stan(file='R and stan code/pois_model_zip3_MRVrerun.stan', data=data,
            iter=40000, warmup=20000, chains=4, thin=1, refresh = 1)
print(out)


# Save all the stan output
saveRDS(out, file="model_out_zip_MRVrerunNONETTERS.rds")
out <- readRDS(file='model_out_zipMRVrerun.rds')
```
