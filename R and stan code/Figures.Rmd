...
```{r, message=FALSE}
EF_Final <- read_csv("EF_DATA_Final.csv")
...
Final_Inset_Map <- ggdraw()+
  draw_plot(EF_Lakes_ZM + ggsn::scalebar(data = MN_Counties, dist = 50, dist_unit = "km", st.size = 2, 
...
zm_diff3 = ZM_Pre3 - ZM_Post3
zm_diff_prop3 = zm_diff3/ZM_Pre3

mean(zm_diff3) #3.736835 walleye per hour
mean(zm_diff_prop3) #40.67%
quantile(zm_diff_prop3, c(0.025, 0.975))

uninvaded_diff3 = NoZM_Pre3 - NoZM_Post3
mean(uninvaded_diff3) #-0.2754049
uninvaded_prop3 = uninvaded_diff3/NoZM_Pre3
mean(uninvaded_prop3)
quantile(uninvaded_prop3, c(0.025, 0.975)) #-0.06585174 or 6.59% increase post
```

Effect of Stocking, stocking led to higher catch rates
```{r}
# Not stocked
Not_stocked <- exp(posterior3[,"b_0"]) 
mean(Not_stocked) # 4.189089
quantile(Not_stocked, c(0.025, 0.975)) # 2.919797 5.754348

#Stocked
stocked <- exp(posterior3[,"b_0"]+posterior3[,"b_stocked"]) 
...
```{r}
final.dat3 <- read_csv("NN_EF_WAE_ZM_Model_3_11.15.21.csv")
...
Model_3_Summary_Lake_Level <- final.dat3 %>%
  mutate(time = ifelse(!is.na(YEAR_INFESTED), YEAR_INFESTED, YEAR_INFESTED.y),
         post = ifelse(year > time, 1, 0))%>%
  dplyr::select(36,37,38,43, 53,54, 60:62)%>%
  group_by(ZM_LAKE)%>%
...
iw <- read_excel("infested-waters_03_21_2023.xlsx", skip = 1)
iw <- iw[, -dim(iw)[2]]
...
wae_lakes <- read_csv("JW ZM code/wae_NR_class.csv")
